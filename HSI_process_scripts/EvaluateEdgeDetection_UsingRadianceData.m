...
AnimalMaskFile = [Directory,'/Masks_',Filename,'.tiff'];
FishMask = imread(AnimalMaskFile,'tiff');
...
for i = 1:16
    Uimg(:,:,i) = RefObjectImg(:,:,i)*Raptor4Cones(1,i); % use Up direction of light field
    Simg(:,:,i) = RefObjectImg(:,:,i)*Raptor4Cones(2,i);
    Mimg(:,:,i) = RefObjectImg(:,:,i)*Raptor4Cones(3,i);
    Limg(:,:,i) = RefObjectImg(:,:,i)*Raptor4Cones(4,i);
...
for i = 1:16
    U_bk(i) = Background(i)*Raptor4Cones(1,i); 
    S_bk(i) = Background(i)*Raptor4Cones(2,i);
    M_bk(i) = Background(i)*Raptor4Cones(3,i);
    L_bk(i) = Background(i)*Raptor4Cones(4,i);
    D_bk(i) = Background(i)*ChickenDoubleCone(i);
    U_White(i) = WhiteSurface(i)*Raptor4Cones(1,i); 
    S_White(i) = WhiteSurface(i)*Raptor4Cones(2,i);
    M_White(i) = WhiteSurface(i)*Raptor4Cones(3,i);
    L_White(i) = WhiteSurface(i)*Raptor4Cones(4,i);
    D_White(i) = WhiteSurface(i)*ChickenDoubleCone(i);
    U_Black(i) = BlackSurface(i)*Raptor4Cones(1,i); 
    S_Black(i) = BlackSurface(i)*Raptor4Cones(2,i);
    M_Black(i) = BlackSurface(i)*Raptor4Cones(3,i);
    L_Black(i) = BlackSurface(i)*Raptor4Cones(4,i);
...
PercentEdge1pixe3 = sum(DetectedEdge3pixel(:))/sum(BWoutlinepixel(:))
...
PercentEdge1pixe3 = sum(DetectedEdge5pixel(:))/sum(BWoutlinepixel(:))
