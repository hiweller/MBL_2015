function [TetraDist, AnimalOnly] = QuantSFlounderConeImages(Directory,Rad4Umat)
% Modified from CC's M-code in 2014
% Example: QuantBuzzardConeImages('ConeImages', 'GlobalRefFile')
load Paralichthys2Cones.dat; % 4x16 (V,S,M,L) % load Paralichthys2Cones.dat; % 1x16
...
for i = 1:16  % use Up direction of light field
    ...
    Limg(:,:,i) = RefObjectImg(:,:,i)*Paralichthys2Cones(2,i); % double cone
end
...
save([Directory, '/JND/SFlounder_', Rad4Umat], 'TetraDist');
