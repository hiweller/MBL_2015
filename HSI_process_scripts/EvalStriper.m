function [PercentEdge1pixel, CtrlEdge1pixel] = EvalStriper(Directory, GlobalRefImg)
...
%   ROTATE MY FRIEND
  ...
  if isempty(find(findfailures)) == 1
      ...
      overlap = ismember(xyrot, Lcoords, 'rows');
      CtrlEdge1pixel(successcount) = length(find(overlap))/length(xyrot(:,1));
  end
