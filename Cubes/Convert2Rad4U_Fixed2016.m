...
%*% vals = Convert2Rad4U('path_to_3D_files/');
...
% load test_ff
% flatfield = ff_test;clear ff_test;
...
if isempty(varargin)
    ...
    flatfieldcorrection_sep_flag = 0;
...
%*% arrangement of the filters on the camera, top left (moving to the right, newline...), to bottom right:
...
%         cuberead(cuberead>2^16) = 0;
    ...
    for j = 1:16
        FlatfieldByBand(:,:,j) = (temp(:,:,ArrayInx(j))); 
    end
...
%         FilterImg = uint16(FilterImg);
    ...
    if radiance_conversion_flag == 1
        ...
        for iband = 1:16
%             FilterImg(:,:,iband) = FilterImg(:,:,iband).*rad_conv(ArrayInx(iband)); %%% !!! the culprit!!!
...
%     BandImg(BandImg<0) = 0;
    ...
    if plot_flag == 1
        ...
        for iplot = 1:16
            subplot(4,4,iplot)
            imagesc(squeeze(BandImg(:,:,iplot)))
            colormap(gray);
            axis square;
            axis off;
            title(WaveNumber{iplot});
        end
