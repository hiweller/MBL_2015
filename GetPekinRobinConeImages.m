function GetPekinRobinConeImages(DateImg,DirImg,DateLight,LightNum,LightDirection)
% Example: GetPekinRobinConeImages('Aug21','2014-08-21-10hr36min06sec-HLgain','Aug7',1,1)
% LightDirection = [up, north, east, south, west, north45, east45, south45, west45]
% LightNum = tree number

load PekinRobin4Cones.dat; % 4x16 (UV,S,M,L)
% load PekinRobinDoubleCone.dat; % 1x16
load ChickenDoubleCone.dat; % 1x16 (very similar to PekinRobinDoubleCone, but more realistic)
PekinRobin4Cones = PekinRobin4Cones/100; % make sensitivity range from 0 to 1
...
for i = 1:16
    Uimg(:,:,i) = RefObjectImg(:,:,i)*LightField(LightDirection,i)*PekinRobin4Cones(1,i); % use Up direction of light field
    Simg(:,:,i) = RefObjectImg(:,:,i)*LightField(LightDirection,i)*PekinRobin4Cones(2,i);
    Mimg(:,:,i) = RefObjectImg(:,:,i)*LightField(LightDirection,i)*PekinRobin4Cones(3,i);
    Limg(:,:,i) = RefObjectImg(:,:,i)*LightField(LightDirection,i)*PekinRobin4Cones(4,i);
    Dimg(:,:,i) = RefObjectImg(:,:,i)*LightField(LightDirection,i)*ChickenDoubleCone(i); % double cone
end
...
for i = 1:16
    U_bk(i) = Background(i)*LightField(LightDirection,i)*PekinRobin4Cones(1,i); 
    S_bk(i) = Background(i)*LightField(LightDirection,i)*PekinRobin4Cones(2,i);
    M_bk(i) = Background(i)*LightField(LightDirection,i)*PekinRobin4Cones(3,i);
    L_bk(i) = Background(i)*LightField(LightDirection,i)*PekinRobin4Cones(4,i);
    D_bk(i) = Background(i)*LightField(LightDirection,i)*ChickenDoubleCone(i);
    U_White(i) = WhiteSurface(i)*LightField(LightDirection,i)*PekinRobin4Cones(1,i); 
    S_White(i) = WhiteSurface(i)*LightField(LightDirection,i)*PekinRobin4Cones(2,i);
    M_White(i) = WhiteSurface(i)*LightField(LightDirection,i)*PekinRobin4Cones(3,i);
    L_White(i) = WhiteSurface(i)*LightField(LightDirection,i)*PekinRobin4Cones(4,i);
    D_White(i) = WhiteSurface(i)*LightField(LightDirection,i)*ChickenDoubleCone(i);
    U_Black(i) = BlackSurface(i)*LightField(LightDirection,i)*PekinRobin4Cones(1,i); 
    S_Black(i) = BlackSurface(i)*LightField(LightDirection,i)*PekinRobin4Cones(2,i);
    M_Black(i) = BlackSurface(i)*LightField(LightDirection,i)*PekinRobin4Cones(3,i);
    L_Black(i) = BlackSurface(i)*LightField(LightDirection,i)*PekinRobin4Cones(4,i);
...
if LightDirection == 1
    OutFilename = [DateImg,'/',DirImg,'/PekinRobin_DCimg_up.jpg'];
    imwrite(DconeAdpNorm,OutFilename,'jpg');
else
...
if LightDirection == 1
    OutFilename = [DateImg,'/',DirImg,'/PekinRobin_DCimg_LoG_up.jpg'];
    imwrite(Dcone_img,OutFilename,'jpg');
else
end
...
if LightDirection == 1
    OutFilename1 = [DateImg,'/',DirImg,'/PekinRobin_LMSimg_up.jpg']; imwrite(LMSimg,OutFilename1,'jpg');
    OutFilename2 = [DateImg,'/',DirImg,'/PekinRobin_UMSimg_up.jpg']; imwrite(UMSimg,OutFilename2,'jpg');
    OutFilename3 = [DateImg,'/',DirImg,'/PekinRobin_LUSimg_up.jpg']; imwrite(LUSimg,OutFilename3,'jpg');
    OutFilename4 = [DateImg,'/',DirImg,'/PekinRobin_LMUimg_up.jpg']; imwrite(LMUimg,OutFilename4,'jpg');
else
end
...
if LightDirection == 1
    OutFilename1 = [DateImg,'/',DirImg,'/PekinRobin_IsoLMSimg_up.jpg']; imwrite(IsoLMSimg,OutFilename1,'jpg');
    OutFilename2 = [DateImg,'/',DirImg,'/PekinRobin_IsoUMSimg_up.jpg']; imwrite(IsoUMSimg,OutFilename2,'jpg');
    OutFilename3 = [DateImg,'/',DirImg,'/PekinRobin_IsoLUSimg_up.jpg']; imwrite(IsoLUSimg,OutFilename3,'jpg');
    OutFilename4 = [DateImg,'/',DirImg,'/PekinRobin_IsoLMUimg_up.jpg']; imwrite(IsoLMUimg,OutFilename4,'jpg');
else
end
...
if LightDirection == 1
    OutFilename1 = [DateImg,'/',DirImg,'/PekinRobin_IsoUcone_LoG_up.jpg']; imwrite(IsoUconeEdge_img,OutFilename1,'jpg');
    OutFilename2 = [DateImg,'/',DirImg,'/PekinRobin_IsoScone_LoG_up.jpg']; imwrite(IsoSconeEdge_img,OutFilename2,'jpg');
    OutFilename3 = [DateImg,'/',DirImg,'/PekinRobin_IsoMcone_LoG_up.jpg']; imwrite(IsoMconeEdge_img,OutFilename3,'jpg');
    OutFilename4 = [DateImg,'/',DirImg,'/PekinRobin_IsoLcone_LoG_up.jpg']; imwrite(IsoLconeEdge_img,OutFilename4,'jpg');
else
end
